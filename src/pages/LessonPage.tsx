import {IonButton, IonContent, IonIcon, IonItem, IonItemDivider, IonLabel, IonList} from "@ionic/react"
...
export const LessonPage = ({history}) => {
  ...
  const continueLearning = () => {
    getNextLesson(id).then(nextLesson => {
      if (nextLesson) {
      ...
      {status === LessonStatus.DONE && <>
        ...
        <div style={{width: "100%"}}>
          {lesson.words.map((word, index) =>
            <div style={{
              backgroundColor: hoveredIndex === index ? "var(--ion-color-light-tint)" : "transparent",
