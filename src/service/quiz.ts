...
for (let i = 0; i < Object.keys(QuizDataType).length; i++) {
  for (let j = i + 1; j < Object.keys(QuizDataType).length; j++) {
    possibleTypes.push({
      a: Object.values(QuizDataType)[i] as QuizDataType,
      b: Object.values(QuizDataType)[j] as QuizDataType,
    });
  }
}
      ...
      return randomWords.some((otherWord, otherIndex) => {
        return (
          ...
          (word.nameRO.includes(otherWord.nameRO) ||
            ...
            otherWord.nameRO.includes(word.nameRO) ||
            otherWord.nameEN.includes(word.nameEN))
        );
      });
    });
    if (!hasDuplicates) {
...
const generateMultipleChoiceQuiz = (
  ...
  if (words.length < context.multipleChoice.numberOfChoices) {
    ...
    randomWords = randomWords.filter(
      word =>
    ...
    words.push(
      ...randomWords
        ...
        .slice(0, context.multipleChoice.numberOfChoices - words.length)
    );
  }
  ...
  if (context.multipleChoice.usedOutputTypes.length % Object.keys(QuizDataType).length === 0) {
    outputType = Object.values(QuizDataType)[
      Math.floor(Math.random() * Object.keys(QuizDataType).length)
    ] as QuizDataType;
    context.multipleChoice.usedOutputTypes = [];
  } else {
    const remainingOutputTypes = Object.values(QuizDataType).filter(
      type => !context.multipleChoice.usedOutputTypes.includes(type as QuizDataType)
    );
    outputType = remainingOutputTypes[
      Math.floor(Math.random() * remainingOutputTypes.length)
    ] as QuizDataType;
  }
  context.multipleChoice.usedOutputTypes.push(outputType);
  // add the other data types to the input types
  const remainingInputTypes = Object.values(QuizDataType).filter(
